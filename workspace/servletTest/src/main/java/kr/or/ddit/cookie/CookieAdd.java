...
public class CookieAdd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 		==> 쿠키값으로 한글을 사용할 경우에는 URLEncoder.encode()메소드로
		// 			인코딩한 후 값을 저장한다.
		
		// 하나의 쿠키는 4kB(4096 byte)까지 저장 가능
		// 하나의 도메인당 20개, 총 300개까 저장 가능
...
//		Cookie ageCookie = new Cookie("age", "26");
		...
		/*
		   ...
		   - 쿠키변수.setMaxAge(유지시간); ==> 단위 : 초 // 
		   			(값이-1이면 웹브라우저가 종료될 때 삭제되고, 값이 0이면 즉시 삭제된다.)
		...
		out.println("<body>");
		out.println("<h2>Cookie 데이터가 저장되었습니다.</h2><hr><br><br>");
