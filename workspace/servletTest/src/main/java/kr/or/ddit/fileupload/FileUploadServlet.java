...
/*
	- Servlet 3.0이상에서 파일을 업로드하려면 서블릿에 @MultipartConfig 애노테이션을 
	  설정해야 한다.
...
public class FileUploadServlet extends HttpServlet {
		...
		for(Part part : request.getParts()) {
			...
			if(!"".equals(fileName)) {
				...
				try {
					...
					fvo.setUploadStatus("Fali : " + e.getMessage());
				}
	...
	// Part객체에서 '파일명'을 찾아서 반환하는 메소드
	/*
		Part 구조
	1) 파일이 아닐 떄 (즉, 일반 Form Data일 경우)
	--------verysksudsdldjlsdf		==> 각각의 Part를 구분하는 구분선
