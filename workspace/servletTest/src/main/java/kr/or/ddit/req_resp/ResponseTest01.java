...
public class ResponseTest01 extends HttpServlet {
	...
	- forward 방식
	  ...
	  사용자(클라이언트)의 웹브라우저의 URL주소가 처음 요청할 때의 값으로 유지된다.
	  
	  서버 내부에서만 접근이 가능하다.
	*/		
		// 이 곳에서 만든 데이터를 이동되는 페이지로 넘길 수 있다.
		// 이 때 사용하는 메소드는 Request객체의 setAttribute()메소드를 사용한다.
		// 형식) request.setAttribute("key값",데이터);
		// 		 key값은 반드시 문자열이어야 하고, 
		//		 데이터는 자바에서 사용할 수 있는 모든 데이터가 가능하다.
		...
		// 	  ==> 객체를 생성할 떄 이동할 서블릿이나 JSP의 주소를 기술하는데
		// 		  전체 URL중에서 ContextPath이후의 내용을 기술하면 된다.
		// http://localhost:8088/servletTest/responseForwardTest.do
