...
public class SetTest {
	...
	*/
		...
		// set에 데이터를 추가할 때도 add()메소드를 사용한다.
		hs1.add("DD");
		hs1.add("AA");
		hs1.add(2);
		hs1.add("CC");
		...
		// 삭제하는 메소드 : remove(삭제할자료) ==> 반환값 : 삭제성공(true) 삭제실패(false)
		// 					 clear() 			==> 전체 자료 삭제
...
//		System.out.println("set 데이터 ==> "+ hs1);
		...
		- Set형의 데이터들을 Interator형 객체로 변환하는 메소드 ==> iterator()
		*/	
		...
		// 			==> Iterator의 데이터를 가리키는 포인터가 있는데 이 포인터가 가리키는 곳 다음번째에 
...
//		int a = random.nextInt(24)+1;
		...
		while (testSet.size()<3) {
			testSet.add(rnd.nextInt(24)+1); 		// 난수값
		}
