...
$.ajax({
 ...
 success : function (res) {
    ...
    let str = "";
       ...
       str += "<input type='button' id='updateBtn' value='수정'/>";
...
function infoUpdate(){
	...
	code += "<tr><td>기본주소 : </td><td> <input tupe='text' id='mem_addr1' class='mem-update' autocomplete=off  ></td></tr>";
	code += "<tr><td>상세주소 : </td><td> <input tupe='text' id='mem_addr2' class='mem-update' autocomplete=off > </td></tr>";
	code += "<tr><td>전화번호 : </td><td> <input tupe='text' id='mem_tel' class='mem-update' autocomplete=off > </td></tr>";
	code += "<tr><td>이메일 	: </td><td> <input tupe='text' id='mem_mail' class='mem-update' autocomplete=off > </td></tr>";

    code += "<tr><td colspan='12' style='text-align: center;'>";
    code += "<input type='button' id='submitBtn' value='저장'>";
    code += "<input type='reset' id='mem-update-cancle' value='취소'/>";
    code += "</tr></table>";
...
//		console.log(memNm);
	...
	let code = "<br>";
	 ...
	 code += "<input type='button' class='button2' id='memWork' name='memWork' onclick='memWork("+memNo+");' value='근태관리'></div>";
...
function mUpdate(data){
	...
	code +=`<input type='button' class='send-cancle-Btn' id='send-cancle' name='send-cancle' value='취소'>`;
...
function showList(){
	...
	$.ajax({
		 ...
		 success : function(res){
		    ...
		    code += "<input type='button' id='resBtn' onclick='res();' value='예약하기'></td></tr></table>"
  ...
  $.ajax({
     ...
     success : function (res) {
        ...
        let str = "";
		 ...
		 str += "<button type='button' class='btn btn-outline-info' id='docConfirm' value='완료' onclick='docBtn(\"완료\")'>완료</button>";
		 str += "<button type='button' class='btn btn-outline-info' id='docWait' value='대기' onclick='docBtn(\"대기\")'>대기</button>"
		 str += "<button type='button' class='btn btn-outline-info' id='docRefuse' value='반려' onclick='docBtn(\"반려\")'>반려</button>"
...
$.ajax({
	...
	success : function(res){
		let str = "";
		str += "<button type-'button' class='btn btn-outline-info' id='docInsertBtn'>추가</button>"
		str += "<button type='button' class='btn btn-outline-info' id='docConfirm' value='완료' onclick='docBtn(\"완료\")'>완료</button>";
		str += "<button type='button' class='btn btn-outline-info' id='docWait' value='대기' onclick='docBtn(\"대기\")'>대기</button>"
		str += "<button type='button' class='btn btn-outline-info' id='docRefuse' value='반려' onclick='docBtn(\"반려\")'>반려</button>"
		str += "<ul>";
...
//		str += "";
	str += "</table>";
	str += "<button class='btn btn-outline-info' type='button' value='추가' id='docuInsert'>추가";
	$('#top').html(str);
...
$.ajax({
	...
	data: {
		"documentSelect" : "휴가신청서",
		"san_per1" : san_per1,		// 직원 1(직원table)
		"san_per2" : san_per2,		// 직원 2(직원table)
		"san_per3" : san_per3,		// 직원 3(직원table)
		"san_deadline"   : san_deadline,		// 결재 마감기한
		"san_nm"   : san_nm,		// 결재 서류 구분
		"san_title": san_title,		// 제목
		"san_content": san_content	// 내용
	},
	dataType: 'json',
	success: function(res){
		console.log(res);
	...
	data : {
	 ...
	 success : function(res){
		...
		$.each(res, function(i,v){
			...
			str += "</ol>";
			str += "<input type='button' class='btn btn-outline-info' class='boardInsertBtn' value='게시글 추가'/>";
...
//  str +="<input type='hidden' id='borNo' name='borNo'value='"+v.board_no+"'>";
 ...
 str += "<button type='button' class='btn btn-outline-info' id='boardinsert'>게시글추가";
...
url : '<%=request.getContextPath() %>/board/board.do',
   ...
   success : function(res){
   	let str = "";
		...
		str += "<br>";
		str += "<button class='btn btn-outline-info' type='button' onclick='freeList(\"\");'>목록</button>";
		str += "<br><hr>";
		str += "</li>";
		str += "</ul><br>";
		...
		str += "<input type='button' class='btn btn-outline-info' class='boardDeleteBtn' value='게시글 삭제' onclick='boardDelete(\""+board_no+"\");'/></div>";
		$('#top').html(str);
   ...
   success : function(res){
   	console.log(res);
		...
		$.each(res, function(i,v){
			...
			str += "<br>";
			str += "<input type='button' class='btn btn-outline-info' class='boardCommentDeleteBtn' value='댓글 삭제' onclick='boardCommentDelete(\""+v.comm_no+"\");'/>";
			str += "</li>";
		});
		str += "</ul><br>";
		//텍스트 추가할까?
		str += "<textarea id='comment' class='comment' name='comment'";
			...
			str += "<div><input type='button' class='btn btn-outline-info' class='boardCommentBtn' value='댓글 추가' onclick='boardCommentInsert(\""+board_no+"\");'>";
...
str += "<input type='button' class='btn btn-outline-info' id='boardInsert' value='추가'></input><br>";
