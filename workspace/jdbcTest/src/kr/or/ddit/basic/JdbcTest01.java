...
// JDBC(java DataBase Connectivity)라일브러리를 이용해서 DB자료를 처리하기
...
public class JdbcTest01 {
	...
	1. 드라이버 로딩 ==> 라이브러리를 사용할 수 있게 메모리에 읽어 들이는 작업
		...
		==> JDBC버전이 4이상에서는 getConnetion()메소드에서 자동으로 로딩해주기
			떄문에 생략할 수 있다. (그렇지만 생략하지 않고 사용할 예정)
	...
	4. 결과 처리 ==> 질의 결과를 받아서 원하는 작업을 수행한다.
		...
		2) 실핸한 SQL문이 select문이 아닐 경우에는 (insert, update, delete)에는 정수값이 반환된다.
		 	(이 정수값은 보통 실행에 성공한 레코드 수를 의미한다.)
	
	5. 사용한 자원 반납한다. ==> close()메소드를 이용한다.
	...
	*/
	public static void main(String[] args) {
		...
		try {
			...
			// 2. DB연결 ==> Connetion객체 생성
			conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe",	// 접속 url 
					"poi", 		// 사용자 ID
					"java");	// 사용자 Password			
