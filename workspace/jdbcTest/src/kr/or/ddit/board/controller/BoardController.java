...
public class BoardController {
	...
	private void boardView() {
		...
		List<BoardVO> getboard = service.getBoard(boardNo);
		
		if (getboard == null || getboard.size() == 0) {
			System.out.println("해당번호의 게시글이 없습니다.");
			startBoard();
		} else {
			for (BoardVO boardVO : getboard) {
				System.out.println(
						"- 제  목 : "+boardVO.getBoard_title() + "\n" +
						"- 작성자 : "+boardVO.getBoard_writer() + "\n" +
						"- 내  용 : "+boardVO.getBoard_content() + "\n" + 
						"- 작성일 : "+boardVO.getBoard_date().toString().split(" ")[0]+ "\n" + 
						"- 조회수 : "+boardVO.getBoard_cnt() );
			}
		}
		System.out.println("-----------------------------------");
